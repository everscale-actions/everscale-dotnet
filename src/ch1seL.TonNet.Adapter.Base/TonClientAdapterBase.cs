...
namespace ch1seL.TonNet.Adapter.Base
{
    public abstract class TonClientAdapterBase : ITonClientAdapter
    {
        ...
        public async Task<TResponse> Request<TResponse>(string method, CancellationToken cancellationToken = default)
            where TResponse : new()
        {
            var responseJson = await Request(method, EmptyJson, null, cancellationToken);
        ...
        {
            var responseJson = await Request(method, EmptyJson, DeserializeCallback(callback), cancellationToken);
            ...
            CancellationToken cancellationToken = default) where TRequest : new() where TResponse : new()
        {
            ...
            var responseJson = await Request(method, requestJson, DeserializeCallback(callback), cancellationToken);
            ...
            CancellationToken cancellationToken = default) where TRequest : new() where TResponse : new()
        {
            ...
            var responseJson = await Request(method, requestJson, null, cancellationToken);
        ...
        {
            var waitDelegatesResult = await WaitForDelegates();
        ...
        private async Task<bool> WaitForDelegates()
        {
            ...
            while (true)
            {
                lock (_lock)
                {
                    if (_requestsDict.Count == 0) return true;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            lock (_lock)
            {
                ...
                _requestsDict.AddOrUpdate(requestId, _ => (tcs, callback), (_, __) => (tcs, callback));
            }
        ...
        {
            return (callbackResponseJson, responseType) =>
            {
                callback?.Invoke(PolymorphicSerializer.Deserialize<TEvent>(callbackResponseJson), responseType);
            };
        }
        ...
        protected void ResponseHandlerBase(uint requestId, string responseJson, uint responseType, bool finished)
        {
            ...
            if (!_requestsDict.TryGetValue(requestId, out var tuple))
            ...
            if (finished) _requestsDict.Remove(requestId, out _);

            var (tcs, callback) = tuple;
