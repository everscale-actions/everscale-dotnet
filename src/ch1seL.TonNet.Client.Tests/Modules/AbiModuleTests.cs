...
namespace ch1seL.TonNet.Client.Tests.Modules
{
    public class AbiModuleTests : TonClientTestsBase
    {
        ...
        public async Task EncodeMessageConstructorSignerExternal()
        {
            ...
            var request = new ParamsOfEncodeMessage
            {
            ...
            ResultOfEncodeMessage unsigned = await TonClient.Abi.EncodeMessage(request);
        ...
        {
            var signature = await TonClient.NaclSignDetachedFromSecretKey(_keys, "KCGM36iTYuCYynk+Jnemis+mcwi3RFCke95i7l96s4Q=");
