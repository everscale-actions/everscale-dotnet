...
namespace ch1seL.TonNet.Client.Modules
{
    public class ProofsModule : IProofsModule
    {
        ...
        public ProofsModule(ITonClientAdapter tonClientAdapter)
        {
        ...
        /// </summary>
        public async Task ProofBlockData(ParamsOfProofBlockData @params, CancellationToken cancellationToken = default)
        {
            await _tonClientAdapter.Request<ParamsOfProofBlockData>("proofs.proof_block_data", @params, cancellationToken);
        }

        /// <summary>
        /// <para>Proves that a given transaction's data, which is queried from TONOS API, can be trusted.</para>
        /// <para>This function requests the corresponding block, checks block proofs, ensures that given transaction</para>
        /// <para>exists in the proven block and compares given data with the proven.</para>
        /// <para>If the given data differs from the proven, the exception will be thrown.</para>
        ...
        /// <para>TONOS API (in this case it is required to provide at least `id` of transaction).</para>
        ...
        /// </summary>
        public async Task ProofTransactionData(ParamsOfProofTransactionData @params, CancellationToken cancellationToken = default)
        {
            await _tonClientAdapter.Request<ParamsOfProofTransactionData>("proofs.proof_transaction_data", @params, cancellationToken);
        }
    }
}