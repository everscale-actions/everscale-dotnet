...
internal class AccumulatorContract : ContractBase {
	...
	public async Task Add(int value, KeyPair keyPair, CancellationToken cancellationToken = default) {
	...
	public async Task Subtract(int value, KeyPair keyPair, CancellationToken cancellationToken = default) {
		await _client.ProcessAndWaitInternalMessages(new ParamsOfEncodeMessage {
			Address = Address,
			Abi = await GetAbi(cancellationToken),
			CallSet = new CallSet {
