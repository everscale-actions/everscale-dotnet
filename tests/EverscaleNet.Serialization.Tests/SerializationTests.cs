...
public class SerializationTests
{
    ...
    public void BigIntegerConverting()
    {
        ...
        json.ShouldBe("{\"bigIng\":1231231231231123123}");
    }
    ...
    public void DoubleNoneField()
    {
        ...
        string accountForExecutorNoneJson =
            JsonSerializer.Serialize(accountForExecutorNone, JsonOptionsProvider.JsonSerializerOptions);

        signerNoneJson.ShouldBe(accountForExecutorNoneJson);
    }
    ...
    public void SerializeEncodeMessageRequestTest()
    {
        ...
        jsonElement.TryGetProperty("abi", out JsonElement abi).ShouldBeTrue();
        abi.TryGetProperty("type", out JsonElement apiType).ShouldBeTrue();
        apiType.GetString().ShouldBe("Contract");

        jsonElement.TryGetProperty("signer", out JsonElement signer).ShouldBeTrue();
        signer.TryGetProperty("type", out JsonElement signerType).ShouldBeTrue();
        signerType.GetString().ShouldBe("External");
        signer.Get<string>("public_key").ShouldBe("4c7c408ff1ddebb8d6405ee979c716a14fdd6cc08124107a61d3c25597099499");
    }
    ...
    public void EnumStringValueType()
    {
        ...
        json.ShouldBe("{\"queries_protocol\":\"WS\"}");
    }
    ...
    {
        AccountType? accountType = JsonDocument.Parse(json).RootElement.TryGet("acc_type", out AccountType result)
            ? result
            : null;
    ...
    public void AccountTypeAnonymousTypeTest(string json, AccountType? expectedAccountType)
    {
        ...
        result.acc_type.ShouldBe(expectedAccountType);
    }

    [Fact]
    public void AccountTypeDeserializationKeyNotFoundExceptionTest()
    {
        var getAccountType = new Action(() => JsonDocument.Parse("{}").RootElement.Get<AccountType>("acc_type"));

        getAccountType.ShouldThrow<KeyNotFoundException>("acc_type is not presented");
    }
    ...
    public void NestedPolymorphicTypeContainsTypeFieldTest()
    {
        ...
        string json =
            JsonSerializer.Serialize<AppRequestResult>(appRequestResult, JsonOptionsProvider.JsonSerializerOptions);

        json.ShouldBe("{\"type\":\"Ok\",\"result\":{\"type\":\"GetPublicKey\",\"public_key\":\"Pubkey\"}}");
    }
    ...
    public void CustomBindingConfigModelMatchesToGenerated()
    {
        ...
        customJson.ShouldBe(generatedJson);
    }
    ...
    public void NestedStructureTest()
    {
        ...
        json.ShouldBe("""{"messages":[{"message":{"type":"Boc","boc":"test"},"wait_until":0}]}""");
    }
