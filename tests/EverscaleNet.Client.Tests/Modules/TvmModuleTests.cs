...
public class TvmModuleTests : IClassFixture<EverClientTestsFixture> {
	...
	private async Task<string> TestRunMessage(Func<ResultOfEncodeMessage, Abi, string, Task<string>> run) {
		...
		string address = await _everClient.DeployWithGiver(encodeMessageParams, cancellationToken: TestContext.Current.CancellationToken);
		var fetchAccount = await FetchAccount(address);
		string account = fetchAccount?.Get<string>("boc");
		...
		// run
		string runResult = await run(message, TestsEnv.Packages.Subscription.Abi, account);

		string getSubscriptionMessage = await GetSubscriptionEncodedMessage(address, TestsEnv.Packages.Subscription,
	...
	private async Task<ResultOfEncodeMessage> GetSubscribeMessage(string address, JsonElement subscribeParams,
		...
		KeyPair keys) {
		ResultOfEncodeMessage subscribeEncodedMessage = await _everClient.Abi.EncodeMessage(new ParamsOfEncodeMessage {
			Address = address,
	...
	private async Task<string> GetSubscriptionEncodedMessage(string address, Package subscriptionPackage,
		...
		ResultOfEncodeMessage getSubscriptionMessage = await _everClient.Abi.EncodeMessage(new ParamsOfEncodeMessage {
			Address = address,
			Abi = subscriptionPackage.Abi,
			CallSet = new CallSet {
	...
	private async Task<JsonElement?> FetchAccount(string address) {
		ResultOfWaitForCollection result = await _everClient.Net.WaitForCollection(new ParamsOfWaitForCollection {
			Collection = "accounts",
	...
	private Func<Task<string>> GetElectorEncodedAccount() {
		return async () =>
		{
			...
			ResultOfEncodeAccount resultOfEncodeAccount =
				await _everClient.Abi.EncodeAccount(paramsOfEncodeAccount, TestContext.Current.CancellationToken);
	...
	public async Task RunAccountNone() {
		...
		ResultOfRunExecutor result = await _everClient.Tvm.RunExecutor(new ParamsOfRunExecutor {
			Message = message,
		...
		ResultOfParse parsed = await _everClient.Boc.ParseAccount(new ParamsOfParse {
			Boc = result.Account
		}, TestContext.Current.CancellationToken);

		parsed.Parsed!.Get<string>("id").ShouldBe("0:f18d106c11586689b11e946269ec1550b69654a8d5964de668149c28877fb65a");
		parsed.Parsed.Get<string>("acc_type_name").ShouldBe("Uninit");
	}

	[Fact]
	public async Task RunAccountUninit() {
		KeyPair keys = await _everClient.Crypto.GenerateRandomSignKeys(TestContext.Current.CancellationToken);
		ResultOfEncodeMessage message = await _everClient.Abi.EncodeMessage(new ParamsOfEncodeMessage {
		...
		parsed.Parsed!.Get<string>("id").ShouldBe(message.Address);
		parsed.Parsed.Get<string>("acc_type_name").ShouldBe("Active");
	}
	...
	public async Task RunExecutor() {
		async Task<string> Run(ResultOfEncodeMessage message, Abi abi, string account) {
			//act
			ResultOfRunExecutor resultOfRun = await _everClient.Tvm.RunExecutor(new ParamsOfRunExecutor {
				Message = message.Message,
				Abi = abi,
			...
			resultOfRun.Transaction!.Get<string>("in_msg").ShouldBe(message.MessageId);
			resultOfRun.Fees.TotalAccountFees.ShouldBeGreaterThan(0);

			return resultOfRun.Account;
		}

		string result = await TestRunMessage(Run);

		result.ToAddress().ShouldBe(SubscribeParamsPubkey);
	}

	[Fact]
	public async Task RunGet() {
		ResultOfRunGet result = await _everClient.Tvm.RunGet(new ParamsOfRunGet {
			Account = await _electorEncodedLazy.Value,
			FunctionName = "participant_list"
		}, TestContext.Current.CancellationToken);

		result.Output!.Value.GetRawText().ShouldBe(
	...
	public async Task RunGet2() {
		...
		result.Output!.ToObject<string[]>().Length.ShouldBe(1);
		result.Output.ToObject<string[]>()[0].ShouldBeEquivalentTo("0");
	}

	[Fact]
	public async Task RunGet3() {
		ResultOfRunGet result = await _everClient.Tvm.RunGet(new ParamsOfRunGet {
			Account = await _electorEncodedLazy.Value,
			FunctionName = "past_elections"
		}, TestContext.Current.CancellationToken);

		result.Output!.Value[0][0][0].GetString().ShouldBe("1588268660");
	}

	[Fact]
	public async Task RunTvm() {
		async Task<string> Run(ResultOfEncodeMessage message, Abi abi, string account) {
			ResultOfRunTvm resultOfRunTvm = await _everClient.Tvm.RunTvm(new ParamsOfRunTvm {
				Message = message.Message,
				Abi = abi,
				Account = account,
				ReturnUpdatedAccount = true
